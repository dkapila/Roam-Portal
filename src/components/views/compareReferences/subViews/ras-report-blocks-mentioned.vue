...
export default {
  name: "ras-report-blocks-mentioned",
  components: {
    BarChart,
  },
  ...
  methods: {
    ...
    onBarChartClicked(event, item) {
      if (item && item.length > 0 && "_index" in item[0]) {
        ...
        if (event.shiftKey) {
          this.roamOpenSidebar(
            clickedItem.uniqueIdentifier.slice(0, -2).slice(2)
          );
          return true;
        }

        this.searchWithWaitingForUserInput = false;
        this.referencedBlockUid = clickedItem.uniqueIdentifier;
        this.displayMode = 0;
      }
    },
    getBarChartTooltipLabel(_, tooltipItem) {
      let tooltipLabel = [];
      let label = `${tooltipItem.yLabel} times this block was used as reference`;
      let labelActionText =
        "Click to show all blocks which contain this block reference";
    ...
    renderBarChart(self) {
      ...
      let chartLabel = [
        "Click to show results which contain this block reference",
      ];
          ...
          xAxes: [
            {
              ...
              scaleLabel: {
                display: true,
                labelString: "Page/block references",
              },
              ticks: {
                callback: (value) => {
                  if (value.length > 16) {
